...
static int notify_user(const char *to_addr, const char *from_addr, const char *message, const char *smtp_server)
{
	CURL *curl;
		...
		int argc, const char **argv)
{
	...
	if (json_object_get(config, "tenant")) {
		tenant = json_string_value(json_object_get(json_object_get(config, "tenant"), "name"));
		if(json_object_get(json_object_get(config, "tenant"), "address")) {
			tenant_addr = json_string_value(json_object_get(json_object_get(config, "tenant"), "address"));
		} else {
	...
	if (json_object_get(config, "smtp_server")) {
		smtp_server = json_string_value(json_object_get(config, "smtp_server"));
	} else {
		fprintf(stderr, "error with Domain in JSON\n");
		return PAM_AUTH_ERR;
	}
	...
	notify_user(user_addr, tenant_addr, prompt, smtp_server);
